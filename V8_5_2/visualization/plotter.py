...
def overlay_colorbar(frame, colorbar_img, margin=20):
    fh, fw = frame.shape[:2]
    ch, cw = colorbar_img.shape[:2]
    ...
    frame[y:y+ch, x:x+cw] = colorbar_img
    return frame, x, y, ch
...
def annotate_colorbar(
    ...
    """
    Draw numeric speed labels aligned with the colorbar gradient.
    """
    ...
    for i in range(n_ticks):
        ...
        # Value mapping (top = vmax)
        value = vmax - frac * (vmax - vmin)

        # Pixel position aligned to gradient
        ty = int(y + pad + frac * usable_height)
